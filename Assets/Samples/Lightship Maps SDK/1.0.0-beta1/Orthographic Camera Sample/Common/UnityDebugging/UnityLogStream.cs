    ...
    public class UnityLogStream : ILogStream
    {
            ...
            switch (logLevel)
            {
                ...
                default:
                    ...
                    Debug.LogErrorFormat(
                        "Invalid log level '{0}' provided to UnityLogStream! Message: {1}", logLevel, fullMessage);
                    break;
            }
        }
        ...
        private string CreateFullMessage(string channel, LogLevel logLevel, string message)
        {
            ...
            if (_settings.UseColorsInEditor)
            {
                ...
                {
                    return string.Format("[{0}] <b><color={1}>[{2}]</color></b> <color={3}>{4}</color>",
                        GetLogLevelStr(logLevel), GetChannelColor(channel), channel, TraceColor, message);
                }

                return string.Format("[{0}] <b><color={1}>[{2}]</color></b> {3}",
                    GetLogLevelStr(logLevel), GetChannelColor(channel), channel, message);
            }
#endif
            ...
            {
                return string.Format("[{0}] [T{1:000000}] [{2}] {3}", 
                    GetLogLevelStr(logLevel), GetElapsedMilliseconds(), channel, message);
            }
        ...
        private string GetChannelColor(string channel)
        {
            if (_isProSkin)
            {
                return LightReadableColors[Math.Abs(channel.GetHashCode()) % LightReadableColors.Length];
            }
