...
namespace Buck
{
    ...
    public class PlaceablesManager : Singleton<PlaceablesManager>
    {
        ...
        [HideInInspector] public bool _saveQueued;
        ...
        private async void RestoreSavedPlaceablesGroups()
        {
            ...
            LoadAll(); // currently experiencing a bug when loading... need to look into it
        ...
        {
            // Debug.Log("Set Current Placeable Triggered");
            ...
            // Debug.Log($"Selected Placeable: {_selectedPlaceable}");
        ...
        {
            // Debug.Log("Finalize Selected Placeable Triggered");
        ...
        {
            // Debug.Log("Queue Save Triggered");
        ...
        {
            // Debug.Log("Save Immediate Triggered");
            ...
            //_placeablesGroups.ForEach(group =>
            foreach (var group in _placeablesGroups)
            {
                groupDataList.Add(group.GroupData);

                // null check added to ensure saving works
                if (group.GroupData == null) continue;
                ...
                group.Placeables.ForEach(placeable =>
                {
                    if (placeable.State == PlaceableState.Finalized)
                    {
                        ...
                        // could pull coordinate data here from each placeable, but might cause inaccuracies based on localization
                    }
        ...
        public void LoadAll()
        {
            ...
            storageData.Groups.ForEach(groupData =>
            {
                ...
                {
                    PlaceableObject placeable = Instantiate(Resources.Load<GameObject>(placeableData.FileName), groupTransform).GetComponent<PlaceableObject>();
        ...
        public async void PlaceObjectOnTerrainAnchorButton()
        {
            await PlaceObjectOnTerrainAnchor(true);
        }

        public async Task<GameObject> PlaceObjectOnTerrainAnchor(bool saveData, GameObject placeable = null)
        {
            if (placeable == null) placeable = _prefabs[0];

            var anchor = await TerrainAnchorManager.Instance.CreateTerrainAnchor();

            _selectedPlaceable = Instantiate(placeable, anchor.pose.position, anchor.pose.rotation);
            Debug.Log($"Object Placed: {placeable.name}");

            var obj = _selectedPlaceable;

            if (saveData)
            {
                var placeableObject = _selectedPlaceable.GetComponent<PlaceableObject>();
                placeableObject.SetPlaceableObjectData(0);

                CreateRemotePlaceablesGroup();
                FinalizeSelectedPlaceable();
            }

            return obj;
        }

        private void CreateRemotePlaceablesGroup()
        {
            var newGroup = Instantiate(_placeablesGroupPrefab, _selectedPlaceable.transform.position, Quaternion.identity).GetComponent<PlaceablesGroup>();
            _placeablesGroups.Add(newGroup);

            newGroup.RegisterPlaceable(_selectedPlaceable.GetComponent<PlaceableObject>());
            newGroup.Init(true);
            
            // detaching from geo anchor to avoid drift on accuracy improved
            // newGroup.DetachWhenTargetReached();
            newGroup.DetachFromGeoAnchor();
        }
